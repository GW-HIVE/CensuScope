...
function TblChart_arrIdx(arr, cols,  icol, frmstart, frmend, maxrowcnt)
{
    ...
    for ( ir=irstart;ir<irend;ir+=jumpRow){
...
function vDir_listPath( container, dir, concat , isdir)
{
...
function  vDir_listDirContent(container, txt)
{
	...
	var lst=txt.split("|");
	var tl="";
	if(curdir.length)tl+="<a href='javascript:vDir_listPath(\""+container+"\",\""+updir+"\",0,1)'>../</a><br>";
	for( i=0 ;i< lst.length; ++i ){
		if(lst[i].length<1)continue;
		var isdir= (lst[i].indexOf("/")!=-1) ? 1 : 0 ;
		var bod="-";
		if(gVDir_listCallbackText) {
			bod=gVDir_listCallbackText(isdir, lst[i], txt, document.forms[gVDir_formName].elements[container].value );
		}
		
		if(bod=="-") { 
			if(isdir)bod="<img border=0  src='img/dir.gif' height=12>&nbsp;"+lst[i];
			else bod=lst[i];
		}
		
		if(!bod.length)continue;
		if(isdir)tl+="<a href='javascript:vDir_listPath(\""+container+"\",\""+lst[i]+"\",1, "+isdir+")'>"+bod+"</a><br>";
		else tl+="<a href='javascript:vDir_listPath(\""+container+"\",\""+lst[i]+"\",1, 0)'>"+bod+"</a><br>";
